...
void light_lows(int i) {
  ...
  if (timenow < drop_time) {
    int thresh_low = 75;
    int colour_int = random(0, 255);
    low_len++;
    if (data_lows[i] < thresh_low) {  
      low_pixel_one = random(0, NUM_LEDS);
      low_pixel_two = low_pixel_one + 5 + low_len;
    }
...
For lows pick increasing pixels starting at 5 ending at 20. Random selection everytime data <
threshold_lows. For lows, the colours should also vary randomly (within a limit??)

Then when we get to the climax, we want darkness followed by a nice colourful light routine.
